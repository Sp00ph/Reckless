...
fn search<NODE: NodeType>(
    ...
    let entry = td.shared.tt.read(hash, td.board.halfmove_clock(), ply, &td.board);
        ...
        && let Some(outcome) = tb::probe(&td.board)
    {
            ...
            || (bound == Bound::Upper && score <= alpha)
        {
            ...
            td.shared.tt.write(hash, depth, Score::NONE, score, bound, Move::NULL, ply, tt_pv, false, &td.board);
            return score;
        }
    ...
    if !(excluded || NODE::ROOT && td.pv_index > 0) {
        td.shared.tt.write(hash, depth, raw_eval, best_score, bound, best_move, ply, tt_pv, NODE::PV, &td.board);
    }
...
fn qsearch<NODE: NodeType>(td: &mut ThreadData, mut alpha: i32, beta: i32, ply: isize) -> i32 {
    ...
    td.shared.tt.write(hash, TtDepth::SOME, raw_eval, best_score, bound, best_move, ply, tt_pv, false, &td.board);
